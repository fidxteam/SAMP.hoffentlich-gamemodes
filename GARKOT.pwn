	...
	if(rows)
  	{
		...
		while(i < rows)
		{
			format(str, sizeof(str), "[ID: %d]\n{ffffff}%s"YELLOW_E"\nPress 'H' to Parkveh/ PickuVeh", id, GetLocation(ppData[id][parkX], ppData[id][parkY], ppData[id][parkZ]));
			...
			//cache_get_value_name_int(i, "pajak", ppData[id][ppajak]);
...
CMD:epepmax(playerid, params[])
{
	...
	new id, query[512];
	if(sscanf(params, "i", id)) return Usage(playerid, "/yahahakenasatir [id]");
	if(!Iter_Contains(Parks, id)) return Error(playerid, "Invalid ID.");
