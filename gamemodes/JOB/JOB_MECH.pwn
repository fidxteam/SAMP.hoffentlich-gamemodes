...
CMD:service(playerid, params[])
{
	if(pData[playerid][pFaction] == 9)
	{
	    if(pData[playerid][pOnDuty] <= 0)
						return Error(playerid, "Kamu harus On duty untuk menyervis!");
		if(pData[playerid][pMechVeh] == INVALID_VEHICLE_ID)
		{
			...
			//if(pData[playerid][pOnDuty] != 1) return ErrorMsg(playerid, "Anda harus duty terlebih dahulu");
		...
		if(GetNearestVehicleToPlayer(playerid, 3.5, false) == pData[playerid][pMechVeh])
		{
	        ...
	        if(pData[playerid][pFactionRank] > 2 && IsPlayerInDynamicArea(playerid, Zonameka))
	        {
...
//Mech JOB
GRP::CheckCar(playerid, vehicleid)
{
...
GRP::BodyFix(playerid, vehicleid)
{
	if(!IsPlayerConnected(playerid)) return 0;
	//if(!IsValidVehicle(vehicleid)) return 0;
	if(pData[playerid][pMechanicStatus] != 1) return 0;
...
GRP::EngineFix(playerid, vehicleid)
{
	if(!IsPlayerConnected(playerid)) return 0;
	//if(!IsValidVehicle(vehicleid)) return 0;
	if(pData[playerid][pMechanicStatus] != 1) return 0;
...
GRP::SprayCar(playerid, vehicleid)
{
	if(!IsPlayerConnected(playerid)) return 0;
	//if(!IsValidVehicle(vehicleid)) return 0;
	if(pData[playerid][pMechanicStatus] != 1) return 0;
...
GRP::PaintjobCar(playerid, vehicleid)
{
	if(!IsPlayerConnected(playerid)) return 0;
	//if(!IsValidVehicle(vehicleid)) return 0;
	if(pData[playerid][pMechanicStatus] != 1) return 0;
...
GRP::ModifCar(playerid, vehicleid)
{
	if(!IsPlayerConnected(playerid)) return 0;
	//if(!IsValidVehicle(vehicleid)) return 0;
	if(pData[playerid][pMechanicStatus] != 1) return 0;
...
GRP::NeonCar(playerid, vehicleid)
{
	if(!IsPlayerConnected(playerid)) return 0;
	//if(!IsValidVehicle(vehicleid)) return 0;
	if(pData[playerid][pMechanicStatus] != 1) return 0;
