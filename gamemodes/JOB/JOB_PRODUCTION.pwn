...
CMD:createproduct(playerid, params[])
{
	...
	if(pData[playerid][pJob] == 6 || pData[playerid][pJob2] == 6)
	{
		...
		|| IsPlayerInRangeOfPoint(playerid, 2.0, -250.88, -2143.23, 29.32) || IsPlayerInRangeOfPoint(playerid, 2.0, -245.74, -2141.65, 29.32))
		{
			...
			{
				if(pData[playerid][pActivityTime] > 5) return Error(playerid, "Anda masih memiliki activity progress!");
				if(Inventory_Count(playerid, "Materials") < 40) return Error(playerid, "Material tidak cukup!(Butuh: 40).");
				if(pData[playerid][CarryProduct] != 0) return Error(playerid, "Anda sedang membawa sebuah product");
				Inventory_Remove(playerid, "Materials", 40);
			...
			else if(type == 3)
			{
				if(pData[playerid][pActivityTime] > 5) return Error(playerid, "Anda masih memiliki activity progress!");
				if(Inventory_Count(playerid, "Materials") < 40) return Error(playerid, "Material tidak cukup!(Butuh: 40).");
				if(Inventory_Count(playerid, "Component") < 20) return Error(playerid, "Component tidak cukup!(Butuh: 20).");
				if(pData[playerid][CarryProduct] != 0) return Error(playerid, "Anda sedang membawa sebuah product");
				Inventory_Remove(playerid, "Materials", 40);
...
GRP::CreateProduct(playerid, type)
{
...
CMD:sellproduct(playerid, params[])
{
	...
	if(pData[playerid][CarryProduct] == 1)
	{
		...
		GivePlayerMoneyEx(playerid, 430);
		
		Product += 10;
		Server_MinMoney(500);
		Info(playerid, "Anda menjual 10 bahan makanan dengan harga "GREEN_E"$430");
	...
	else if(pData[playerid][CarryProduct] == 2)
	{
		...
		Product += 10;
		Server_MinMoney(500);
		Info(playerid, "Anda menjual 10 product baju dengan harga "GREEN_E"$450");
		pData[playerid][pJobTime] += 480;
	}
	else if(pData[playerid][CarryProduct] == 3)
	{
		...
		Product += 10;
		Server_MinMoney(630);
		Info(playerid, "Anda menjual 10 product equipment dengan harga "GREEN_E"$500");
		pData[playerid][pJobTime] += 480;
	}
	return 1;
}
