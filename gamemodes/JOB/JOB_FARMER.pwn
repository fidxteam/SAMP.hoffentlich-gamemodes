...
function HarvestPlant(playerid)
{
	if(pData[playerid][pHarvestID] != -1)
	{
		...
		if(pData[playerid][pActivityTime] >= 100)
		{
			...
			{
				Inventory_Add(playerid, "Marijuana", 1578, kg);
...
CMD:sellmarijuana(playerid, params[])
{
	...
	if(Inventory_Count(playerid, "Marijuana") < 1) return ErrorMsg(playerid, "You dont have marijuana");
	new pay = Inventory_Count(playerid, "Marijuana") * MarijuanaPrice;
	...
	//Inventory_Count(playerid, "Marijuana") = 0;
