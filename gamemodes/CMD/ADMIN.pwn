CMD:acmds(playerid)
{
	...
	{
		strcat(line3, "{ff0000}[Helper]: {ffffff}/quiz question, /quiz answer,  /adminjail, /togooc /goto, /freeze, /unfreeze, /slap, /caps, /aitems, /astats, /adelays\n");
	...
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"Hoffentlich:RP: "YELLOW_E"Staff Commands", line3, "OK","");
	return true;
}
...
CMD:setfamilly(playerid, params[])
{
	...
	if(sscanf(params, "ud", otherid, famid))
		...
		Servers(playerid, "You have sepName],family ID %d with Member.", pData[otherid][pName], famid);
...
CMD:allduty(playerid, params[])
{
	if(pData[playerid][pAdmin] < 26)
			return PermissionError(playerid);
	...
	foreach(new i : Player)
	{
	    if(pData[i][pAdmin] > 0)
	    {
			if(!strcmp(pData[i][pAdminname], "None"))
				return ErrorMsg(i, "Kamu harus setting Nama Admin mu!");

			if(!pData[i][pAdminDuty])
		    {
					SetPlayerColor(i, 0xFF000000);
					pData[i][pAdminDuty] = 1;
					SetPlayerName(i, pData[i][pAdminname]);
					SendStaffMessage(COLOR_RED, "* %s telah on duty admin.", pData[i][pName]);
		    }
		    else
		    {
		        ...
		        pData[i][pAdminDuty] = 0;
		        SetPlayerHealth(i, 100.0);
		        SendStaffMessage(COLOR_RED, "* %s telah off admin duty.", pData[i][pName]);
		    }
		}
	}
	return 1;
...
CMD:offduty(playerid, params[])
{
	...
	if(pData[otherid][pFaction] != -1 && pData[otherid][pOnDuty])
		            SetFactionColor(otherid);
		        else
		            SetPlayerColor(otherid, COLOR_WHITE);

		        SetPlayerName(otherid, pData[otherid][pName]);
		        pData[otherid][pAdminDuty] = 0;
		        SetPlayerHealth(otherid, 100.0);
		        SendStaffMessage(COLOR_RED, "* %s telah off admin duty.", pData[otherid][pName]);
	return 1;
}
...
CMD:sendplayer(playerid, params[])
{
	...
	new otherid, tujuan;
	if(sscanf(params, "ud", otherid, tujuan))
        return Usage(playerid, "/sendplayer [playerid/PartOfName] [player tujuan]");

    if(!IsPlayerConnected(otherid) || !IsPlayerConnected(tujuan))
        return ErrorMsg(playerid, "Player belum masuk!");

	SendPlayerToPlayer(otherid, tujuan);
	Servers(tujuan, "%s telah di tp ke kamu oleh admin.", pData[otherid][pName]);
	Servers(otherid, "You has teleport to %s position.", pData[tujuan][pName]);
	return 1;
}
...
CMD:canda(playerid, params[])
{
	if(pData[playerid][pAdmin] < 20)
			return PermissionError(playerid);

	new Float:POS[3], otherid, canda;
	if(sscanf(params, "ud", otherid, canda))
	{
	    Usage(playerid, "/slap <ID> <Type>");
	    return true;
	}
	...
	{
		GetPlayerPos(otherid, POS[0], POS[1], POS[2]);
		SetPlayerPos(otherid, POS[0], POS[1], POS[2] + 1000.0);
		if(IsPlayerInAnyVehicle(otherid))
		{
			RemovePlayerFromVehicle(otherid);
			//OnPlayerExitVehicle(otherid, GetPlayerVehicleID(otherid));
...
function OWarnPlayer(adminid, NameToWarn[], warnReason[])
{
	if(cache_num_rows() < 1)
	{
		return ErrorMsg(adminid, "Account {ffff00}'%s' "WHITE_E"does not exist.", NameToWarn);
	}
	else
	{
	    new RegID, warn;
...
CMD:respawnjobs(playerid, params[])
{
	...
	for(new xx;xx<sizeof(BaggageVeh);xx++)
	{
		if(IsVehicleEmpty(BaggageVeh[xx]))
		{
			...
			SetVehicleFuel(BaggageVeh[xx], 100);
	...
	for(new xx;xx<sizeof(MowerVeh);xx++)
	{
		if(IsVehicleEmpty(MowerVeh[xx]))
		{
			...
			SetVehicleFuel(MowerVeh[xx], 100);
...
CMD:afix(playerid, params[])
{
    ...
    if(IsPlayerInAnyVehicle(playerid)) 
	{
        SetValidVehicleHealth(GetPlayerVehicleID(playerid), 10000);
        SetVehicleFuel(GetPlayerVehicleID(playerid), 100);
...
CMD:ans(playerid, params[])
{
	...
	if(!IsPlayerConnected(to_player))
		return ErrorMsg(playerid, "This player is not conect");

	if(!strlen(message))
		return ErrorMsg(playerid, "Masukkan Pesan");
...
CMD:ann(playerid, params[])
{
	...
	pData[playerid][pInfo] += 1;
		...
		if(pData[playerid][pInfo] == 3)
		{
		    ...
		    pData[playerid][pInfo3] = 2;
		}
		if(pData[playerid][pInfo] == 4)
		{
		    pData[playerid][pInfo4] = 2;
		    format(strings, sizeof(strings), "%s", params);
...
CMD:unbanip(playerid, params[])
{
	...
	if(sscanf(params, "s[32]", IP))
	    return ErrorMsg(playerid, "/unbanip [IP]");
...
IPRP::OnUCPBanned(playerid, ucp[], reason[])
{
...
CMD:setfaction(playerid, params[])
{
    ...
    if(sscanf(params, "udd", otherid, fid, rank))
        return Usage(playerid, "/setfaction [playerid/PartOfName] [1.SAPD, 2.SAGS, 3.SAMD, 4.SANEW 5.PEDAGANG 6.POLANTAS 7.BRIMOB 8.SAAF 9.SAAG] [rank 1-6]");
    ...
    if(fid < 0 || fid > 9)
        return ErrorMsg(playerid, "You have specified an invalid faction ID 0 - 7.");
		
	if(rank < 1 || rank > 13)
        return ErrorMsg(playerid, "You have specified an invalid rank 1 - 12.");
...
CMD:takebmoney(playerid, params[])
{
	...
	if(!IsPlayerConnected(otherid))
 	...
 	if(bmoney > pData[otherid][pBankMoney])
		return ErrorMsg(playerid, "Player doesn't have enough bmoney to deduct from!");
...
CMD:setmoney(playerid, params[])
{
	...
	new dc[2000];
    format(dc, sizeof(dc),  "```\n[ADMIN LOGS] %s[UCP: %s] mengset uang kepada %s[UCP: %s] sebesar %s```", pData[playerid][pAdminname], pData[playerid][pUCP], pData[otherid][pName], pData[otherid][pUCP], FormatMoney(money));
...
CMD:givemoney(playerid, params[])
{
	...
	new dc[2000];
    format(dc, sizeof(dc),  "```\n[ADMIN LOGS] %s[UCP: %s] memberikan uang kepada %s[UCP: %s] sebesar %s```", pData[playerid][pAdminname], pData[playerid][pUCP], pData[otherid][pName], pData[otherid][pUCP], FormatMoney(money));
    SendDiscordMessage(9, dc);
...
CMD:anticheat(playerid, params[])
{
	...
	if(pData[playerid][pAdmin] < 5)
	{
	    return ErrorMsg(playerid, "Kamu tidak diizinkan untuk menggunakan command ini.");
	}
	...
	if(IsPlayerConnected(playerid)) 
	{
		...
		{			
			if(sscanf(param, "i", ammount))
			{
				...
				Info(playerid, "Names: <1> Taxi, <2> deleted, <3> LumberJack, <4> Trucker, <5> Miner");
				Info(playerid, "Names: <6> Production, <7> Farmer, <8> Kurir, <9> Smuggler, <10> Baggage <11> pemotong");
				return 1;
			}
			if(!(0 <= ammount <= 11))
			{
				return ErrorMsg(playerid, "Invalid job.");
			}
...
CMD:checkdc(playerid, params[])
{
	...
	if(sscanf(params, "s[24]", name))
	{
	    Usage(playerid, "/checkdc <ucpname>");
 		return 1;
 	}

    new characterQuery[178];
	mysql_format(g_SQL, characterQuery, sizeof(characterQuery), "SELECT * FROM `playerucp` WHERE `ucp` = '%s'", name);
	mysql_tquery(g_SQL, characterQuery, "Checkdc", "is", playerid, name);
	return true;
}

forward Checkdc(playerid, Nama_UCP[]);
public Checkdc(playerid, Nama_UCP[])
{
	...
	if(rows > 0)
	{
		...
		format(dc, sizeof(dc),  "Discord id: %s", ID);
		SendClientMessageEx(playerid, -1, dc);
	}
	else
	{
		SendClientMessageEx(playerid, -1, "Ucp tidak ada");
	}
	return 1;
}
CMD:setadminmoderator(playerid, params[])
{
	...
	if(pData[otherid][pAdmin] < 1)
		return ErrorMsg(playerid, "Player tersebut bukan team pengurus, set terlebih dahulu!");
...
CMD:moderatorhelp(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "{FF0000}Moderator Commands | Hoffentlich Roleplay", string, "Close","");
	return 1;
}
...
{
	if(pData[playerid][pAdmin] < 5) return ErrorMsg(playerid, "Permisioon Error.");

	new vehicleid = GetPlayerVehicleID(playerid);
	if(!IsValidVehicle(vehicleid)) return ErrorMsg(playerid, "Kendaraan Ini Tidak Bisa Di Hapus!");
	{
	    DestroyVehicle(vehicleid);
	}
	return 1;
