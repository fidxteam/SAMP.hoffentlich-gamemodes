...
GetClosestParks(playerid, Float: range = 4.3)
{
	...
	foreach(new i : Parks)
	{
	    tempdist = GetPlayerDistanceFromPoint(playerid, ppData[i][parkXX], ppData[i][parkYY], ppData[i][parkZZ]);
	...
	if(rows)
  	{
		...
		while(i < rows)
		{
			format(str, sizeof(str), "'Klakson' to parkveh");
			...
			cache_get_value_name_float(i, "posxx", ppData[id][parkXX]);
			cache_get_value_name_float(i, "posyy", ppData[id][parkYY]);
			cache_get_value_name_float(i, "poszz", ppData[id][parkZZ]);
			cache_get_value_name_int(i, "interior", ppData[id][parkInt]);
			cache_get_value_name_int(i, "world", ppData[id][parkWorld]);
			ppData[id][parkPickup] = CreateDynamicObject(1316, ppData[id][parkX], ppData[id][parkY], ppData[id][parkZ] - 0.5, 0.000000, 0.000000, 0.000000, -1, -1, -1, 300.00, 300.00);
			ppData[id][parkpoint] = CreateDynamicObject(1316, ppData[id][parkXX], ppData[id][parkYY], ppData[id][parkZZ] - 0.5, 0.000000, 0.000000, 0.000000, -1, -1, -1, 300.00, 300.00);
			SetDynamicObjectMaterial(ppData[id][parkPickup], 0, 18871, "matcolours", "green", 0xFF00FF33);
			SetDynamicObjectMaterial(ppData[id][parkpoint], 0, 18870, "matcolours", "red", 0xFFFF0000);
			ppData[id][parkpsp] = CreateDynamicSphere(ppData[id][parkX], ppData[id][parkY], ppData[id][parkZ], 2);
			ppData[id][parkssp] = CreateDynamicSphere(ppData[id][parkXX], ppData[id][parkYY], ppData[id][parkZZ], 2);
			ppData[id][parkicon] = CreateDynamicMapIcon(ppData[id][parkX], ppData[id][parkY], ppData[id][parkZ], 55, -1, -1, -1, -1, 3000.0);
			ppData[id][parkLabel] = CreateDynamic3DTextLabel(str, ARWIN, ppData[id][parkXX], ppData[id][parkYY], ppData[id][parkZZ]+0.3, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, ppData[id][parkWorld], ppData[id][parkInt], -1, 10.0);
			Iter_Add(Parks, id);
	    	i++;
		}
...
{
	new cQuery[512];
	format(cQuery, sizeof(cQuery), "UPDATE parks SET posx='%f', posy='%f', posz='%f', posxx='%f', posyy='%f', poszz='%f',interior=%d, world=%d WHERE id=%d",
	ppData[id][parkX],
	ppData[id][parkY],
	ppData[id][parkZ],
	ppData[id][parkXX],
...
CMD:createpark(playerid, params[])
{
	...
	ppData[id][parkPickup] = CreateDynamicObject(1316, ppData[id][parkX], ppData[id][parkY], ppData[id][parkZ] - 0.5, 0.000000, 0.000000, 0.000000, -1, -1, -1, 300.00, 300.00);
	SetDynamicObjectMaterial(ppData[id][parkPickup], 0, 1316, "matcolours", "white", 0xFF00FF33);
	ppData[id][parkicon] = CreateDynamicMapIcon(ppData[id][parkX], ppData[id][parkY], ppData[id][parkZ], 55, -1, -1, -1, -1, 3000.0);
	Iter_Add(Parks, id);
...
GetNearbyGarkott(playerid)
{
for(new i = 0; i < MAX_PARKPOINT; i ++)
{
	if(IsPlayerInRangeOfPoint(playerid, 3.0, ppData[i][parkXX], ppData[i][parkYY], ppData[i][parkZZ]))
	 {
		return i;
	 }
		}
...
CMD:setparkpos(playerid, params[])
{
	...
	new str[128];
	ppData[id][parkpoint] = CreateDynamicObject(1316, ppData[id][parkXX], ppData[id][parkYY], ppData[id][parkZZ] - 0.5, 0.000000, 0.000000, 0.000000, -1, -1, -1, 300.00, 300.00);
	SetDynamicObjectMaterial(ppData[id][parkpoint], 0, 1316, "matcolours", "white", 0xFFFF0000);
	format(str, sizeof(str), "[ID: %d]\n{ffffff}%s"YELLOW_E"\n'Klakson' to Parkveh", id, GetLocation(ppData[id][parkXX], ppData[id][parkYY], ppData[id][parkZZ]));
	ppData[id][parkLabel] = CreateDynamic3DTextLabel(str, ARWIN, ppData[id][parkXX], ppData[id][parkYY], ppData[id][parkZZ] + 0.3, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, ppData[id][parkWorld], ppData[id][parkInt], -1, 10.0);
	Park_Save(id);
	return 1;
}
...
CMD:removepark(playerid, params[])
{
	...
	DestroyDynamicMapIcon(ppData[id][parkicon]);
