...
CMD:creategarage(playerid, params[])
{
	...
	new dc[2000];
    format(dc, sizeof(dc),  "```\n[ADMIN LOGS] %s[UCP: %s] telah membuat garasi [Location: %s | ID: %d]```", pData[playerid][pAdminname], pData[playerid][pUCP], GetLocation(ppData[id][parkX], ppData[id][parkY], ppData[id][parkZ]), id);
    SendDiscordMessage(9, dc);
	return 1;
}

GRP::OnParkCreated(playerid, id)
{
	Park_Save(id);
...
CMD:setgpos(playerid, params[])
{
	...
	Park_Save(id);
	new dc[2000];
    format(dc, sizeof(dc),  "```\n[ADMIN LOGS] %s[UCP: %s] telah memindahkan lokasi garasi [Location: %s | ID: %d]```", pData[playerid][pAdminname], pData[playerid][pUCP], GetLocation(ppData[id][parkX], ppData[id][parkY], ppData[id][parkZ]), id);
    SendDiscordMessage(9, dc);
	return 1;
}
...
CMD:removegarage(playerid, params[])
{
	...
	new dc[2000];
    format(dc, sizeof(dc),  "```\n[ADMIN LOGS] %s[UCP: %s] telah menghapus garasi [Location: %s | ID: %d]```", pData[playerid][pAdminname], pData[playerid][pUCP], GetLocation(ppData[id][parkX], ppData[id][parkY], ppData[id][parkZ]), id);
    SendDiscordMessage(9, dc);
	return 1;
}
