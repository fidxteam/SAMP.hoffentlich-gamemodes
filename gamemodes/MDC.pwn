...
{
    if(pData[playerid][pCheckingBis] != 1) return 0;
	{
...
forward melacak(playerid, to_player, number);
public melacak(playerid, to_player, number)
{
	pData[playerid][pMelacak] = 1;
...
public trackph(playerid, to_player, number)
{
	...
	{
	    KillTimer(pData[playerid][pLacak]);
	}
	return 1;
}
