...
CMD:createlocker(playerid, params[])
{
	...
	new type;
	if(sscanf(params, "d", type)) return Usage(playerid, "/createlocker [type, 1.KEPOLISIAN KOTAKU 2.SAGS 3.EMS 4.REPORTER 5.PEDAGANG 6.VIP 7.KEPILOTAN 8.MECHANIC]");
...
CMD:editlocker(playerid, params[])
{
    ...
    else if(!strcmp(type, "type", true))
    {
        ...
        if(sscanf(string, "d", tipe))
            return Usage(playerid, "/editlocker [id] [type] [type, 1.KEPOLISIAN KOTAKU 2.SAGS 3.EMS 4.REPORTER 5.PEDAGANG 6.VIP]");
