...
CMD:winvite(playerid, params[])
{
    ...
    if(sscanf(params, "ud", otherid, rank))
        return Usage(playerid, "/invite [playerid/PartOfName] [Pangkat]");
	...
	if(pData[otherid][pWorkshop] != -1)
		return Error(playerid, "Player tersebut sudah berkerja di workshop!");

	pData[otherid][pWoInvite] = pData[playerid][pWorkshop];
	pData[otherid][pWoRank] = rank;
	pData[otherid][pWoOffer] = playerid;
	Servers(playerid, "Anda telah menginvite %s untuk menjadi anggota workshop.", pData[otherid][pName]);
	Servers(otherid, "%s telah menginvite anda untuk menjadi anggota workshop. Type: /accept workshop or /deny workshop!", pData[playerid][pName]);
	return 1;
}
...
IPRP::ListFamilys(playerid, ListFamily)
{
	new rows = cache_num_rows();
 	if(rows)
  	{
...
IPRP::ShowFamilyMember(playerid)
{
...
IPRP::ShowWorkshopMember(playerid)
{
	new rows = cache_num_rows(), pid, username[50], wrank, query[1048];
 	if(rows)
  	{
		for(new i = 0; i != rows; i++)
		{
			...
			if(wrank == 1)
			{
				strcat(query, ""FAMILY_E"Magang(1)");
			}
			else if(wrank == 2)
			{
				strcat(query, ""FAMILY_E"Pegawai(2)");
			}
			else if(wrank == 3)
			{
				strcat(query, ""FAMILY_E"Karyawan(3)");
			}
			else if(wrank == 4)
			{
				strcat(query, ""FAMILY_E"Manager(4)");
			}
			else if(wrank == 5)
			{
				strcat(query, ""FAMILY_E"Owner(5)");
			}
