...
Bisnis_BuyMenu(playerid, bizid)
{
    ...
    switch(bData[bizid][bType])
    {
        ...
        case 2:
        {
            format(string, sizeof(string), "Produk\tHarga\n{ffffff}Snack\t{7fff00}%s\n{ffffff}Sprunk\t{7fff00}%s\n{ffffff}Gas Fuel\t{7fff00}%s\n{ffffff}Bandage\t{7fff00}%s\n{ffffff}Camping\t{7fff00}$300\n{ffffff}Tali\t{7fff00}$1000\n{ffffff}Karung\t{7fff00}$500",
                FormatMoney(bData[bizid][bP][0]),
                FormatMoney(bData[bizid][bP][1]),
                FormatMoney(bData[bizid][bP][2]),
                FormatMoney(bData[bizid][bP][3])
...
Bisnis_Type(bisid)
{
	...
	if(bData[bisid][bType] == 3) //Clothes
	{
	    switch(random(1))
		{
			case 0:
			{
				bData[bisid][bIntposX] = 207.05;
				bData[bisid][bIntposY] = -138.80;
				bData[bisid][bIntposZ] = 1003.50;
				bData[bisid][bIntposA] = 22.92;
				bData[bisid][bInt] = 3;
			}
			/*case 1:
			{
...
Bisnis_Refresh(id)
{
    if(id != -1)
    {
        ...
        {
            format(string, sizeof(string), "[ID: %d]\n{00FF00}This bisnis for sell\n{FFFFFF}Location: {FFFF00}%s\n{FFFFFF}Price: {FFFF00}%s\n{FFFFFF}Type: {FFFF00}%s\n"WHITE_E"Type /buybiz to purchase", id, GetLocation(bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ]), FormatMoney(bData[id][bPrice]), type);
            bData[id][bPickup] = CreateDynamicPickup(19133, 23, bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ]+0.2, 0, 0, _, 25.0);
			bData[id][bPickup2] = CreateDynamicPickup(19133, 23, bData[id][bIntposX], bData[id][bIntposY], bData[id][bIntposZ]+0.2, id, bData[id][bInt], _, 25.0);
		}
...
CMD:tokobaju(playerid, params[])
{
	foreach(new i : Bisnis)
	{
	    if(bData[i][bType] == 3)
	    {
			bData[i][bIntposX] = 207.05;
				bData[i][bIntposY] = -138.80;
				bData[i][bIntposZ] = 1003.50;
				bData[i][bIntposA] = 22.92;
				bData[i][bInt] = 3;
		}
	}
}
...
IPRP::OnBisnisCreated(playerid, bid)
{
	Bisnis_Type(bid);
    Bisnis_Refresh(bid);
...
IPRP::GetClosestBusiness(playerid, type)
{
	new
    ...
    foreach(new bid : Bisnis)
		{
			if(IsPlayerInRangeOfPoint(playerid, 2.5, bData[bid][bExtposX], bData[bid][bExtposY], bData[bid][bExtposZ]))
			{
				...
				{
				    #if LIMIT_PER_PLAYER > 0
					if(Player_BisnisCount(playerid) + 1 > 4) return ErrorMsg(playerid, "You can't buy any more bisnis.");
					#endif
				}
				else
				{
...
CMD:givebisnis(playerid, params[])
{
	...
	new dc[10000];
    format(dc, sizeof(dc),  "```\n[GIVE BUSINNES] %s[UCP: %s] memberikan bussines [ID: %d] kepada %s[UCP: %s]```", GetRPName(playerid), pData[playerid][pUCP], bid, GetRPName(otherid), pData[otherid][pUCP]);
