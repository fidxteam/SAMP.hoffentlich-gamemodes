...
IPRP::OnPlayerDataLoaded(playerid, race_check)
{
	...
	{		
		...
		if(pData[playerid][pPassword] > 10)
		{
			...
			ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD, "Login To Hoffentlich", lstring, "Login", "Quit");
		}
	}
	else
	{
...
IPRP::CheckBanAccount(playerid, namachar[])
{
	if(cache_num_rows() > 0)
	{
		new Reason[40], PlayerName[24], BannedName[24];
...
IPRP::CheaterKontolMemek(playerid)
{
...
IPRP::AssignPlayerData(playerid)
{
	...
	cache_get_value_name_int(0, "sidejob_korantime", pData[playerid][pKoranTime]);
	cache_get_value_name_int(0, "sidejob_antarayamtime", pData[playerid][pAntarayamTime]);
	...
	cache_get_value_name_int(0, "tali", pData[playerid][pTali]);
	cache_get_value_name_int(0, "sehat", pData[playerid][pSehat]);
	cache_get_value_name_int(0, "regen", pData[playerid][pRegen]);
	cache_get_value_name_int(0, "tiket", pData[playerid][pTiket]);
	cache_get_value_name_int(0, "camping", pData[playerid][pCamping]);
	...
	new query[218];
	format(query, sizeof(query), "SELECT * FROM `inventory` WHERE `ID` = '%d'", pData[playerid][pID]);
    mysql_tquery(g_SQL, query, "LoadInventory", "d", playerid);
	...
	/*foreach(new ii : Player)
	{
		...
		{
			SendClientMessageEx(ii, COLOR_RED, "{ff0000}[SERVER]:"YELLOW_E" %s[ID:%d] connected to the server.", pData[playerid][pName], playerid);
		}
	}*/
	new dc[150];
	/*format(dc, sizeof(dc),  "```\n%s connected to the server.```", pData[playerid][pName]);
	SendDiscordMessage(0, dc);*/
	format(dc, sizeof(dc),  "%s has join the server.", pData[playerid][pName]);
//	SendDiscordMessage(0, dc);
	...
	format(str1, sizeof str1, "Informasi %s\nRegister Id: %d\nIp : %s\nNama Ucp : %s", pData[playerid][pName], pData[playerid][pID], pData[playerid][pIP], pData[playerid][pUCP]);
	DCC_SetEmbedDescription(embed, str1);
	DCC_SetEmbedColor(embed, 0x00ff01);
	...
	/*for(new i = 0; i < 8; i++)
	{
		TextDrawShowForPlayer(playerid, SPAWNFIVEM[i]);
...
IPRP::CekNamaDobelJing(playerid, name[])
{
...
IPRP::OnPlayerRegister(playerid)
{
...
IPRP::PhoneNumber(playerid, phone)
{
	if(cache_num_rows() > 0)
	{
		//Rekening Exist
...
IPRP::_KickPlayerDelayed(playerid)
{
	Kick(playerid);
	return 1;
}

IPRP::SafeLogin(playerid)
{
...
IPRP::InfoTD_MSG(playerid, ms_time, text[])
{
...
//---------[ Twitter IPRP::]---------
IPRP::ChangeTwitUserName(otherplayer, nname[])
{
	if(cache_num_rows() > 0)
	{
		// Name Exists
...
IPRP::a_ChangeTwitterName(otherplayer, playerid, twname[])
{
	if(cache_num_rows() > 0)
	{
		// Name Exists
...
IPRP::a_ChangeAdminName(otherplayer, playerid, nname[])
{
	if(cache_num_rows() > 0)
	{
		// Name Exists
...
IPRP::LoadStats(playerid, PlayersName[])
{
	if(!cache_num_rows())
	{
...
IPRP::CheckPlayerIP(playerid, zplayerIP[])
{
...
IPRP::CheckPlayerIP2(playerid, zplayerIP[])
{
...
IPRP::OnOBanQueryData(adminid, NameToBan[], banReason[], banTime)
{
	new mstr[512];
	mstr = "";
	if(!cache_num_rows())
...
IPRP::DragUpdate(playerid, targetid)
{
...
IPRP::RefullCar(playerid, vehicleid)
{
...
IPRP::SetPlayerToUnfreeze(playerid, Float:x, Float:y, Float:z, Float:a)
{
...
IPRP::KickTimer(playerid)
{
	KickEx(playerid);
	return 1;
}

IPRP::CheckPlayerSpawn3Titik(playerid)
{
	...
	{
		SetPlayerPos(playerid, 1647.7306,-2286.5720,-1.2138);
...
IPRP::CheckUCP(playerid, nameucp[])
{
	new count = cache_num_rows(), datez, line[248], tstr[64], lstr[128];
	if(count)
	{
