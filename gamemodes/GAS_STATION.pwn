...
IPRP::OnGstationCreated(playerid, gsid)
{
	GStation_Save(gsid);
	new str[150];
...
CMD:fill(playerid, params[])
{
	...
	Mobilisi[playerid] = GetNearestVehicleToPlayer(playerid);
	if(!IsEngineVehicle(Mobilisi[playerid]))
            return Error(playerid, "You are not in engine vehicle.");

	if(GetEngineStatus(Mobilisi[playerid]))
					return Error(playerid, "Turn off vehicle engine.");

	if(GetVehicleFuel(Mobilisi[playerid]) >= 1000.0)
		return Error(playerid, "This vehicle gas is full.");

	if(pData[playerid][pFill] != -1)
		return Error(playerid, "You already filling vehicle. please wait!");
		
	if(pData[playerid][pMoney] <= 100) return ErrorMsg(playerid, "Uang lu kurg atau 100 dollar pas minimal 100 keatas lah");
	...
	foreach(new gsid : GStation)
	{
		if(IsPlayerInRangeOfPoint(playerid, 4.0, gsData[gsid][gsPosX], gsData[gsid][gsPosY], gsData[gsid][gsPosZ]))
		{
			...
			pData[playerid][pFill] = gsid;
			pData[playerid][pFillStatus] = 1;
			new Float:Value = GetVehicleFuel(Mobilisi[playerid])*30.0/125;
			PlayerTextDrawTextSize(playerid, FuelTD[playerid][2], 250.0, -Value);
			for(new i = 0; i < 8; i++)
			{
		 		PlayerTextDrawShow(playerid, FuelTD[playerid][i]);
			}
			Sedangngisi[playerid] = SetTimerEx("Fuelloading", 1000, true, "d", playerid);
		}
	}
	return 1;
}

CMD:testfuel(playerid, params[])
{
 SendClientMessageEx(playerid, -1, "%d", Mobilisi[playerid]);
}
stock Fillloading(playerid)
{
	new Float:Value = GetVehicleFuel(Mobilisi[playerid])*30.0/125;
	PlayerTextDrawTextSize(playerid, FuelTD[playerid][2], 250.0, -Value);
	PlayerTextDrawShow(playerid, FuelTD[playerid][2]);
	new str[128];
	format(str, sizeof str, "%d", GetVehicleFuel(Mobilisi[playerid]));
	PlayerTextDrawSetString(playerid, FuelTD[playerid][5], str);
	PlayerTextDrawShow(playerid, FuelTD[playerid][5]);
	return 1;
}

forward Fuelloading(playerid);
public Fuelloading(playerid)
{
	new old = GetVehicleFuel(Mobilisi[playerid]);
		    SetVehicleFuel(Mobilisi[playerid], old + 10);
	Fillloading(playerid);
	pData[playerid][pFillPrice] += GStationPrice;
			gsData[pData[playerid][pFill]][gsStock] -= 10;
			GivePlayerMoneyEx(playerid, -10*GStationPrice);
    foreach(new gsid : GStation)
	{
		if(GetPlayerMoney(playerid) < GStationPrice*10 || GetEngineStatus(Mobilisi[playerid]))
		{
			...
			GStation_Refresh(gsid);
			//Info(playerid,"uang kurang atau kendaraan menyala pengisian berhenti total "RED_E"%s.", FormatMoney(pData[playerid][pFillPrice]*10));
			Mobilisi[playerid] = 0;
	...
	if(GetVehicleFuel(Mobilisi[playerid]) >= 1000 )
	{
		KillTimer(Sedangngisi[playerid]);
		//HideProgressBar(playerid);
		for(new i = 0; i < 8; i++)
		...
		GStation_Refresh(gsid);
		InfoMsg(playerid,"Tangki sudah penuh pengisian berhenti total "RED_E"%s.", FormatMoney(pData[playerid][pFillPrice]*10));
		Mobilisi[playerid] = 0;
		KillTimer(pData[playerid][pFillTime]);
		pData[playerid][pFillStatus] = 0;
		pData[playerid][pFillPrice] = 0;
...
IPRP::Filling(playerid)
{
	...
	foreach(new gsid : GStation)
	{
		...
		else
		{
			...
			pData[playerid][pFillPrice] += GStationPrice;
			gsData[pData[playerid][pFill]][gsStock] -= 10;
			if(GetVehicleFuel(GetPlayerVehicleID(playerid)) >= 100.0)
			{
