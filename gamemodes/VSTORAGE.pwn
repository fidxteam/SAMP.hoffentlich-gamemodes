...
Trunk_OpenStorage(playerid, vehicleid)
{
	...
	format(string, sizeof(string), "Weapon Storage [%d/5]\nMoney Safe [%s]\nComponent [%d/8000]\nMaterial [%d/8000]\nMarijuana [%d/8000]\nSnack [%d/150]\nSprunk [%d/150]\nGas [%d/5]", items[0], FormatMoney(vsData[vehicleid][tMoney]), vsData[vehicleid][tComp], vsData[vehicleid][tMats], vsData[vehicleid][tMarijuana], vsData[vehicleid][tSnack], vsData[vehicleid][tSprunk], vsData[vehicleid][tGas]);
...
IPRP::LoadVehicleTrunk(vehicleid)
{
 	...
 	if(rows)
  	{
		for(new z = 0; z < rows; z++)
		{
			...
			cache_get_value_name_int(z, "gas", vsData[vehid][tGas]);
			cache_get_value_name_int(z, "sprunk", vsData[vehid][tSprunk]);
			cache_get_value_name_int(z, "snack", vsData[vehid][tSnack]);
...
Trunk_Save(vehicleid)
{
	...
	mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`gas`= %d,", cQuery, vsData[x][tGas]);
	mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`sprunk`= %d,", cQuery, vsData[x][tSprunk]);
	mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`snack`= %d,", cQuery, vsData[x][tSnack]);
...
IPRP::MySQL_CreateVehicleStorage(vehicleid)
{
	new query[512];
	...
	vsData[vehicleid][tGas] = 0;
	vsData[vehicleid][tSnack] = 0;
	vsData[vehicleid][tSprunk] = 0;
