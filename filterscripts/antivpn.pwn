...
public MyHttpResponse(playerid, response_code, data[])
{
	...
	{	
		if(data[0] == 'Y')
		{
			format(string, 256, "[LunarPride:Anti-VPN] %s(%d) has been kicked from the server.", name, playerid);
	    	SendClientMessageToAll( 0xFF0000FF, string);
	    	SendClientMessage(playerid, 0xFF0000FF, "_________Harap Matikan VPN!_________");
		...
		if(data[0] == 'X')
		{
			printf("IP FORMAT SALAH");
		}
		else
		{
			printf("PERMINTAAN GAGAL, CODE RESPONSE: %d", response_code);
		}
	}
	return 1;
}
