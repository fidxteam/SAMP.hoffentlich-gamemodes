    ...
    if(dialogid == 9000)
    {
        ...
        switch(listitem)
        {
            case 0:
            {
                ...
                PlayerTextDrawHide(playerid, VOICE_3[playerid]);
                Info(playerid, "Normal");
            }
            case 1:
            {
                localStream[playerid] = SvCreateDLStreamAtPlayer(5.0, SV_INFINITY, playerid);
                PlayerTextDrawShow(playerid, VOICE_1[playerid]);
                PlayerTextDrawHide(playerid, VOICE_2[playerid]);
                PlayerTextDrawHide(playerid, VOICE_3[playerid]);
                Info(playerid, "Berbisik");
            }
            case 2:
            {
                localStream[playerid] = SvCreateDLStreamAtPlayer(35.0, SV_INFINITY, playerid);
                PlayerTextDrawShow(playerid, VOICE_1[playerid]);
                PlayerTextDrawShow(playerid, VOICE_2[playerid]);
                PlayerTextDrawShow(playerid, VOICE_3[playerid]);
